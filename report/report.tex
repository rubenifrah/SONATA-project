...
% --- Title Information ---
\title{\textbf{Reproducibility and Critical Analysis of SONATA: \\ Self-Supervised Learning of Reliable Point Representations}}
...
In this report, we focus on one central question: do Sonata's final dense point-level features remain strongly predictive of absolute coordinates, despite the architecture's goal of reducing the geometric shortcut? To investigate this, we design a probing and correlation pipeline, validate it on synthetic controls, and apply it to dense features extracted from an unseen indoor point cloud on the MesoNET computing cluster. We then discuss what these results suggest about the construction of dense 3D SSL representations and the open question of whether feature up-casting preserves or reintroduces spatial dependence.

This report investigates the following questions:
\begin{enumerate}
    \item Can absolute spatial coordinates be linearly recovered from Sonata's final dense representations?
...
To bridge this resolution gap, the authors employ a parameter-free feature up-casting mechanism. Coarse features are progressively mapped back to the original point density using inverse pooling indices, and then concatenated with earlier, higher-resolution feature maps. We hypothesize that this mechanism may preserve or reintroduce dependence on absolute spatial structure. The later experiments are designed to test whether the final dense features remain strongly predictive of coordinates. 
...
\subsection{Hypothesis 2: Pure 3D SSL May Face a Geometric Ceiling in Photometrically Ambiguous Regions}

This second hypothesis is not directly tested in the present report; rather, it serves as a broader interpretive lens for discussing the limits of purely geometric self-supervised learning. Beyond architectural mechanisms, purely 3D self-supervised models face an inherent data modality limitation, which we term the ``geometric ceiling.'' The Sonata pretext tasks rely heavily on spatial structural perturbations and masked point predictions to derive object semantics. However, in human-centric indoor environments, semantics are frequently decoupled from structural geometry. 

Consider a flat wall adorned with a whiteboard, a paper poster, and a flat-screen television. Geometrically, these objects are nearly indistinguishable---they are all planar surfaces with minimal structural variance. A purely geometric SSL model like Sonata lacks the dense photometric texture required to differentiate them, inevitably clustering them into a single semantic group. 
...
All experiments were conducted on the MesoNET (Juliet) computing cluster. Feature extraction was performed using a single NVIDIA A100 GPU (80GB) to accommodate the 108M parameter PointTransformerV3 (PTv3) Sonata backbone. The evaluation pipeline, including the linear regression probes and PCA projections, was executed on high-performance CPU nodes. The software environment utilized PyTorch 2.5.0 with CUDA 12.4.
...
An important mathematical nuance emerges when comparing the linear probe to the correlation metrics. While the linear probe $R^2$ score is nearly perfect ($0.988$), the maximum absolute Pearson correlation remains relatively low ($\approx 0.219$). This discrepancy reveals the nature of Sonata's geometric leakage: it is highly distributed. 
